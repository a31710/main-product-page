...
export default function Pagination({
  ...
  return (
    <div className="flex flex-col md:flex-row items-start md:items-center gap-3 md:gap-4 w-full md:w-auto">
      ...
      <div className="flex items-center gap-1 md:gap-1.5 w-full md:w-auto">
        <button
          onClick={() => onPageChange(Math.max(1, currentPage - 1))}
          disabled={currentPage === 1}
          className="px-2 md:px-3 py-1 md:py-1.5 border border-gray-300 rounded-md text-xs font-semibold text-gray-700 hover:bg-purple-50 hover:border-purple-300 hover:text-purple-600 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-white disabled:hover:border-gray-300 disabled:hover:text-gray-700 transition flex items-center gap-1"
        >
          <ChevronLeft className="w-3 h-3 md:w-3.5 md:h-3.5" />
          <span className="hidden sm:inline">Previous</span>
        </button>

        <div className="flex items-center gap-0.5 md:gap-1 flex-1 justify-center md:flex-initial md:justify-start">
          {renderPageNumbers().map((p, index) => (
            <button
              ...
              disabled={p === "..."}
              className={clsx(
                "w-7 h-7 md:w-8 md:h-8 rounded-md text-xs font-semibold transition",
                p === currentPage
